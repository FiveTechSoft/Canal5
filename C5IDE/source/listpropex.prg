...
DEFINE WINDOW oWnd1 TITLE "Properties" FROM Aplicacion():nTopProp,    Aplicacion():nLeftProp ;
                                         TO Aplicacion():nBottomProp, Aplicacion():nRightProp PIXEL
Aplicacion():oInspector := oWnd1
oWnd1:ToolWindow()

@ 0,0 COMBOBOX oCbxComponentes VAR cVar ITEMS aItems SIZE 300,400 PIXEL OF oWnd1 FONT oFont


oList := TListPropEx():New( 2, 2, 300, 500, , {"","",""}, ;
               {20, 100, 100}, oWnd1, , , ,,;
               ,, oFont )
...
//New(2,2,300,300,,{"Browse"},,oWnd,,,,,,,,,,,,.F.,,.F.,,.f.,,,)
oList:nLineStyle := 2
/*
nGroup := oList:AddGroup( "Appearence" )

oList:AddItem( ,"3D Look", .f.,,nGroup )
oList:AddItem( ,"Absolute Align", .f.,,nGroup )
oList:AddItem( ,"Caption", "Exportación a Excel",,nGroup )
oList:AddItem( ,"Client Edge", .f.,,nGroup )
oList:AddItem( ,"Clip Children", .f.,,nGroup )
oList:AddItem( ,"Clip Siblings", .f.,,nGroup )
oList:AddItem( ,"Horizontal Scrollbar", .f.,,nGroup )
oList:AddItem( ,"Layout RTL", .f.,,nGroup )
oList:AddItem( ,"Left Scrollbar", .f.,,nGroup )
oList:AddItem( ,"Maximize Box", .f.,,nGroup )
oList:AddItem( ,"Minimize Box", .f.,,nGroup )
oList:AddItem( ,"Overlaped Window", .f.,,nGroup )
oList:AddItem( ,"Palette Window", .f.,,nGroup,,.f. )
oList:AddItem( ,"Static Edge", .f.,,nGroup )
oList:AddItem( ,"Style", "Overlapped",,nGroup )
oList:AddItem( ,"System Menu", .t.,,nGroup )
oList:AddItem( ,"Title Bar", .f.,,nGroup )
oList:AddItem( ,"Tool Window", .f.,,nGroup )
oList:AddItem( ,"Topmost", .f.,,nGroup )
oList:AddItem( ,"Transparent", .f.,,nGroup )
oList:AddItem( ,"Vertical ScrollBar", .f.,,nGroup )
oList:AddItem( ,"Window Edge", .f.,,nGroup )
...
oList:AddItem( ,"Italic", .f.,,nGroup )
oList:AddItem( ,"Underline", .f.,,nGroup )
oList:AddItem( ,"Strike", .f.,,nGroup )
oList:AddItem( ,"Bold", .f.,,nGroup )



nGroup := oList:AddGroup( "Behavior" )
oList:AddItem( ,"Accept Files", .f.,,nGroup )
oList:AddItem( ,"Application Window", .f.,,nGroup )
oList:AddItem( ,"Disabled", .f.,,nGroup )
oList:AddItem( ,"NoInheritLayout", .f.,,nGroup )
oList:AddItem( ,"Right To Left Reading Order", .f.,,nGroup )
oList:AddItem( ,"Set Foreground", .f.,,nGroup )
oList:AddItem( ,"System Modal", .f.,,nGroup )
oList:AddItem( ,"Visible", .t.,,nGroup )


nGroup := oList:AddGroup(  "Position" )
oList:AddItem( ,"Center", .t.,,nGroup )
oList:AddItem( ,"X Pos", 0,,nGroup )
oList:AddItem( ,"Y Pos", 0,,nGroup )
*/
...
CLASS TListPropEx FROM TWBrowse2
      ...
      METHOD AddItem( cVARNAME, cText, nTipo, nGroup, lAbierto, lEnable, bAction )
      METHOD GetLine()
...
METHOD New( nRow, nCol, nWidth, nHeigth, bLine, aHeaders, ;
               ...
               bLDblClick, bRClick, oFont, oCursor, nClrFore,;
               nForeBack, cMsg, lUpdate, cAlias, lPixel, bWhen,;
               lDesign, bValid, bLClick, aActions ) CLASS TListPropEx
...
DEFAULT bValid := {||.t.}
       ...
       ::oBrush:End()
       oBar:SetColor( 0, RGB(240,240,240))
...
*********************************************************************
    METHOD aGetRectSel() CLASS TListPropEx
*********************************************************************
...
***********************************************************************************************************************
   METHOD AddItem( cVARNAME, cText, nTipo, nGroup, lAbierto, lEnable, bAction, bEditor ) CLASS TListPropEx
***********************************************************************************************************************
DEFAULT lAbierto := .t.
DEFAULT nTipo    := ""
DEFAULT nGroup   := 1
...
//aadd( ::aItems, { cVARNAME, nTipo, cText, uVal, nGroup, lAbierto, ::nItem, lEnable, bAction } )
...
*********************************************************************
      METHOD GetLine() CLASS TListPropEx
*********************************************************************
local aLine := {"",""}
...
*********************************************************************
      METHOD nGetElement( n ) CLASS TListPropEx
*********************************************************************
...
*********************************************************************
      METHOD CalcLen() CLASS TListPropEx
*********************************************************************
...
************************************************************************************
   METHOD Open( lOpen ) CLASS TListPropEx
************************************************************************************
local nItem := ::nGetElement( ::nAt )
local oProp
...
************************************************************************************
   METHOD OpenClose( nRow, nCol ) CLASS TListPropEx
************************************************************************************
local nItem := ::nGetElement( ::nAt )
local lOpen := .t.
local oProp
...
************************************************************************************
   METHOD Click( nRow, nCol ) CLASS TListPropEx
************************************************************************************
local nItem := ::nGetElement( ::nAt )
...
if oProp:lEnable
   ...
   if !empty( ::aRectBtn )
      ...
      if PtInRect( nRow, nCol, ::aRectBtn ) .or. nRow < 0
         do case
            ...
            case oProp:cType == LISTA
                 ...
                 DEFINE FONT oFont NAME "Tahoma" SIZE 10, aRect[3]-aRect[1]-8
                 ...
                 ::oGet:bLostFocus := {|| ::oGet:VarPut( If( cType == "L", ( uVar == aItems[ 1 ] ), ::oGet:VarGet() ) ) ,;
                 If( Self:nLastKey != VK_ESCAPE, Eval( bValid, uVar, Self:nLastKey, Self ), Eval( bValid, nil, Self:nLastKey, Self ) ),;
                 oProp:Set(::oGet:VarGet()),::oGet:End(), ::oObject:Refresh() }

                 ::oGet:bKeyDown := { | nKey | If( nKey == VK_RETURN .or. nKey == VK_ESCAPE,( Self:nLastKey := nKey, oProp:Set(::oGet:VarGet()),::oGet:End(),o:SetFocus(), ::oObject:Refresh()), 1  ) }
      ...
      if nCol > ::aColSizes[1]+::aColSizes[2] .and. oProp:cType != GRUPO
...
************************************************************************************
   METHOD GetAction( nItem ) CLASS TListPropEx
************************************************************************************
...
************************************************************************************
   METHOD Teclas( nKey, nFlags ) CLASS TListPropEx
************************************************************************************
local nItem := ::nGetElement(::nAt)
local oProp := ::aItems[nItem]
local lGrupo := oProp:cType == GRUPO
...
do case
   case nKey == VK_SPACE
        if lGrupo
           ...
           if oProp:cType == LISTA
              ::Click(-1)
           else
              if oProp:cType == LOGICO
                 ::Click( -1 )
                 ::DrawSelect()
              endif
           endif
...
METHOD wBrwLine( hWnd, hDC, nRowPos, aValues, aColSizes,;
                          nColPos, nClrText, nClrPane,;
                          hFont, lTree, aJustify, nPressed, nLineStyle,;
                          nColAct, lFocused, oVScroll, bLogicLen, lBar ) CLASS TListPropEx
************************************************************************************
...
****************************************************************************************
  METHOD PaintBtn( hDC, aRect, nItem ) CLASS TListPropEx
****************************************************************************************
local oProp := ::aItems[nItem]
local hBmp
...
do case
   case oProp:cType == LOGICO
        ...
        DrawFrameControl( hDC, aRect, 4, if(oProp:Get(),1024,0)  )//+16384
        ::aRectBtn := aRect
...
return nil
******************************************************************************
      METHOD OpenAll() CLASS TListPropEx
******************************************************************************
local nItem
for nItem := 1 to len( ::aItems )
    if ::aItems[nItem] == GRUPO
...
return nil
******************************************************************************
      METHOD CloseAll() CLASS TListPropEx
******************************************************************************
local nItem
for nItem := 1 to len( ::aItems )
    if ::aItems[nItem] == GRUPO
...
****************************************************************************************************************
  METHOD EditCol( uVar, nItem, cPicture, bValid, nClrFore, nClrBack, aItems,bAction ) CLASS TListPropEx
****************************************************************************************************************
   ...
   ::oGet:bLostFocus := {|| ( ::oGet:Assign(),;
                              ::oGet:VarPut( if( valtype(::oGet:oGet:VarGet()) == "C", alltrim(::oGet:oGet:VarGet()), ::oGet:oGet:VarGet())),;
                              OSend( ::oObject, "_" + oProp:cProperty, ::oGet:oGet:VarGet())) ,;
                              If( Self:nLastKey != VK_ESCAPE,;
                                  Eval( bValid, uVar, Self:nLastKey, Self ),;
                                  Eval( bValid, nil, Self:nLastKey, Self ) ),;
      ::oGet:End(), ::oObject:Refresh() }
...
****************************************************************************************
  METHOD SetInspect( oObject ) CLASS TListPropEx
****************************************************************************************
local n, nItem, nLen
...
METHOD ReDefine( nId, bLine, oDlg, aHeaders, aColSizes, cField, uVal1, uVal2,;
                 ...
                 bWhen, bValid, bLClick, aActions ) CLASS TWBrowse2
...
METHOD DrawIcons() CLASS TWBrowse2
...
METHOD Edit( nCol, lModal ) CLASS TWBrowse2
...
METHOD ReSize( nSizeType, nWidth, nHeight ) CLASS TWBrowse2
...
METHOD SetArray( aArray ) CLASS TWBrowse2
...
METHOD SetTree( oTree ) CLASS TWBrowse2
...
METHOD lEditCol( nCol, uVar, cPicture, bValid, nClrFore, nClrBack,;
                 aItems, bAction ) CLASS TWBrowse2
...
METHOD GoRight() CLASS TWBrowse2
...
METHOD GoBottom() CLASS TWBrowse2
...
METHOD LDblClick( nRow, nCol, nKeyFlags ) CLASS TWBrowse2
...
METHOD LButtonDown( nRow, nCol, nKeyFlags ) CLASS TWBrowse2
...
METHOD LButtonUp( nRow, nCol, nFlags ) CLASS TWBrowse2
...
METHOD KeyDown( nKey, nFlags ) CLASS TWBrowse2
...
METHOD KeyChar( nKey, nFlags ) CLASS TWBrowse2
...
METHOD PageUp( nLines ) CLASS TWBrowse2
...
METHOD PageDown( nLines ) CLASS TWBrowse2
...
METHOD VScroll( nWParam, nLParam ) CLASS TWBrowse2
...
METHOD HScroll( nWParam, nLParam ) CLASS TWBrowse2
...
METHOD _RecCount( uSeekValue ) CLASS TWBrowse2
...
METHOD Report( cTitle, lPreview ) CLASS TWBrowse2
...
METHOD UpStable() CLASS TWBrowse2
...
METHOD SetFilter( cField, uVal1, uVal2 ) CLASS TWBrowse2
...
METHOD MouseMove( nRow, nCol, nKeyFlags ) CLASS TWBrowse2
...
METHOD MouseWheel( nKeys, nDelta, nXPos, nYPos ) CLASS TWBrowse2
...
METHOD VertLine( nColPos, nColInit ) CLASS TWBrowse2
...
METHOD nAtCol( nColPixel ) CLASS TWBrowse2
...
METHOD nAtIcon( nRow, nCol ) CLASS TWBrowse2
...
METHOD Display() CLASS TWBrowse2
...
METHOD GetDlgCode( nLastKey ) CLASS TWBrowse2
...
METHOD SetCols( aData, aHeaders, aColSizes ) CLASS TWBrowse2
...
METHOD ShowSizes() CLASS TWBrowse2
...
METHOD _DrawIcon( nIcon, lFocused ) CLASS TWBrowse2
...
METHOD IsColVisible( nCol ) CLASS TWBrowse2
...
METHOD wBrwLine( hWnd, hDC, nRowPos, aValues, aColSizes,;
                          nColPos, nClrText, nClrPane,;
                          hFont, lTree, aJustify, nPressed, nLineStyle,;
                          nColAct, lFocused, oVScroll, bLogicLen, lBar ) CLASS TWBrowse2
...
//   if valtype( aValues[1]) == "N" .and. aValues[1] != 0
      ...
      if ValType( aValues[ n ] ) == "N" .or. nRowPos == 0
         ...
         if ! lTree
            ...
            if aJustify != nil .and. aJustify[ n ]
               ...
               DrawText( hDC, cValue, { nTop, nLeft+3, nBottom, nRight + If( (nLineStyle == ;
                           LINES_NONE .or. nLineStyle == LINES_H_GRAY .or. ;
...
CLASS TProperty
      ...
      METHOD New( oObject, cProperty, cText, bProperty, cType, lEnable, lAbierto, bEditor, nItem, nGroup ) CONSTRUCTOR

      METHOD Set( uValue ) INLINE oSend( ::oObject,"_" + ::cProperty, uValue )
      METHOD Get( )        INLINE if( ::cType != GRUPO, oSend( ::oObject, ::cProperty ),"")
      METHOD Valtype()     INLINE Valtype( ::Get() )


ENDCLASS

******************************************************************************************************************************
  METHOD New( oObject, cProperty, cText, bProperty, cType, lEnable, lAbierto, bEditor, nItem, nGroup ) CLASS TProperty
******************************************************************************************************************************

DEFAULT cType := ""

//? oObject, cProperty, cText, bProperty, cType, lEnable, lAbierto, bEditor, nItem, nGroup
