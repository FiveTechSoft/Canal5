...
CLASS TPrinter
   ...
   METHOD New( cDocument, lUser, lMeta, cModel, lModal, lSelection, aRect ) CONSTRUCTOR
   ...
   METHOD C5Say( nRow, nCol, cText, oFont, nWidth, nHeight, nClrText, lTrans, nAlign, nGiro, nClrPane )
   ...
   METHOD SayImage( nRow, nCol, oImage, nWidth, nHeight, nRaster, lTransparent )
   ...
   METHOD C5Box( nRow, nCol, nBottom, nRight, oPen, lTrans, nClrPane, ;
                 lGrad, nClrPane2, cTypeGrad, lRoundRect, lMidRound, lOnlyCorners,;
             nRW, nRH )
...
METHOD New( cDocument, lUser, lMeta, cModel, lModal, lSelection, aRect ) CLASS TPrinter
...
METHOD _StartPage() CLASS TPrinter
   ...
   if ::lMeta
       ...
       ::hDCOut := C5CreateEnhMetaFile( ::hDC, ATail( ::aMeta ), ::cDocument, ::aRect )  //jlcr
    #else
...
*********************************************************************************************************
   METHOD C5Say( nRow, nCol, cText, oFont, nWidth, nHeight, nClrText, lTrans, nAlign, nGiro, nClrPane ) CLASS TPrinter
*********************************************************************************************************
 ...
 DrawText( hDC, cText, {nT, nL, nB, nR}, if( nAlign != 4,36,0) + {DT_LEFT,DT_CENTER,DT_RIGHT,0}[nAlign]+DT_NOCLIP  )
...
METHOD SayImage( nRow, nCol, oImage, nWidth, nHeight, nRaster, lTransparent ) CLASS TPrinter

   local hDib, hPalBmp, hPal, nRatio, n

   DEFAULT nWidth := 0, nHeight := 0, lTransparent := .f.
   ...
   do case
      ...
      case valtype( oImage ) == "N"
           hDib := DibFromBitmap( oImage )
      otherwise
           hDib = 0
   endcase
   ...
   if valtype( oImage ) == "N"
      if nWidth == 0 .and. nHeight > 0 .and. ( n := BmpHeight(oImage) ) > 0
         nRatio := BmpWidth(oImage) / n
         nWidth := int( nHeight * nRatio )
      elseif nWidth > 0 .and. nHeight == 0 .and. ( n := BmpWidth(oImage) ) > 0
         nRatio  := BmpHeight(oImage) / n
   ...
   if lTransparent
      TransparentBlt( ::hDCOut, oImage, nRow, nCol, nWidth, nHeight, RGB(255,0,255) )
   else
...
DEFAULT nClrPane     := CLR_WHITE
DEFAULT lGrad        := .f.
DEFAULT lRoundRect   := .f.
DEFAULT lMidRound    := .f.
DEFAULT lOnlyCorners := .f.

if lTrans
   hOldBrush := SelectObject( ::hDCOut, GetStockObject( 5 ) )
else
...
if lGrad
   if cTypeGrad == "Vertical"
      VerticalGradient( ::hDCOut, {nRow, nCol, nBottom, nRight}, nClrPane, nClrPane2 )
   else
      HorizontalGradient( ::hDCOut, {nRow, nCol, nBottom, nRight}, nClrPane, nClrPane2 )
   endif
else
